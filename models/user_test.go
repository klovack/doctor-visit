...
func testUserToManyDoctors(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	_, err = tx.Exec("insert into \"favorite_doctors\" (\"user_id\", \"doctor_id\") values ($1, $2)", a.UserID, b.DoctorID)
	if err != nil {
		t.Fatal(err)
	}
	_, err = tx.Exec("insert into \"favorite_doctors\" (\"user_id\", \"doctor_id\") values ($1, $2)", a.UserID, c.DoctorID)
	if err != nil {
		t.Fatal(err)
	}

	check, err := a.Doctors().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, v := range check {
		if v.DoctorID == b.DoctorID {
			bFound = true
		}
		if v.DoctorID == c.DoctorID {
			cFound = true
		}
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadDoctors(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Doctors); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.Doctors = nil
	if err = a.L.LoadDoctors(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Doctors); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyUserIllnesses(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := UserSlice{&a}
	if err = a.L.LoadUserIllnesses(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testUserToManyVisits(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := UserSlice{&a}
	if err = a.L.LoadVisits(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testUserToManyAddOpDoctors(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		...
		if a.R.Doctors[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.Doctors[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.Doctors().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
func testUserToManySetOpDoctors(t *testing.T) {
	var err error
	...
	err = a.SetDoctors(ctx, tx, false, &b, &c)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.SetDoctors(ctx, tx, true, &d, &e)
	if err != nil {
		t.Fatal(err)
	}
...
func testUserToManyRemoveOpDoctors(t *testing.T) {
	var err error
	...
	err = a.AddDoctors(ctx, tx, true, foreigners...)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.RemoveDoctors(ctx, tx, foreigners[:2]...)
	if err != nil {
		t.Fatal(err)
	}
...
func testUserToManyAddOpUserIllnesses(t *testing.T) {
	var err error
...
func testUserToManySetOpUserIllnesses(t *testing.T) {
	var err error
...
func testUserToManyRemoveOpUserIllnesses(t *testing.T) {
	var err error
...
func testUserToManyAddOpVisits(t *testing.T) {
	var err error
...
func testUserToManySetOpVisits(t *testing.T) {
	var err error
...
func testUserToManyRemoveOpVisits(t *testing.T) {
	var err error
...
var (
	userDBTypes = map[string]string{`UserID`: `integer`, `FirstName`: `character varying`, `LastName`: `character varying`, `Email`: `character varying`, `BirthDate`: `date`}
	_           = bytes.MinRead
)
